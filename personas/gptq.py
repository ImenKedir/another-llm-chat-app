# # Run 'any model' with AutoGPTQ
#
# In this example, we run a quantized 4-bit version of 'any model' using
...
# [Running 'any model' with `bitsandbytes` quantization](/docs/examples/falcon_bitsandbytes). You can also
...
def download_model():
    ...
    model_name = "TheBloke/Chronos-Hermes-13B-SuperHOT-8K-GPTQ"
...
stub = Stub(name="chronos-hermes-gptq-8k", image=image)
...
@web_endpoint()
def streaming_endpoint(prompt: str):
    ...
    return StreamingResponse(
        chain(
            model.generate.remote_gen(prompt_template.format(prompt)),
        ),
