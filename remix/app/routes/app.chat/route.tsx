...
export default function Chat() {
  ...
  function SendMessage(event: React.FormEvent) {
    ...
    const url = `${data.infrence_api_url}/completion?prompt=${encodeURIComponent(prompt)}`;
    const sse = new EventSource(url, { withCredentials: true });

    // handle incoming tokens
    sse.addEventListener("message", (event) => {
      const token = JSON.parse(event.data).text as string;
      setStreamingResponse((prevStreamingResponse) => prevStreamingResponse + token);
    });
  ...
  return (
    <div className={styles.container}>
      <Form onSubmit={SendMessage}>
          ...
          <button disabled={isLoading} type="submit">Ask</button>
      </Form>
